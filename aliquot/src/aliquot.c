...
int main(void){
        ...
        if (num ==1)// ΑΝ ΕΙΝΑΙ 1 ΤΟΤΕ ΤΟ ΑΘΡΟΙΣΜΑ ΕΙΝΑΙ 0 
        {sum=0;}
        else 
        {
            for (long long i=2;i*i<=num ;i++ )//ΕΛΕΓΧΕΙ ΟΛΟΥΣ ΤΟΥΣ ΔΙΑΙΡΕΤΕΣ ΜΕΧΡΙ ΤΗΝ ΤΕΤΡΑΓΩΝΙΚΗ ΡΙΖΑ
            {
                ...
                {
                    sum += i;
                    if(i!=num/i)//ΕΛΕΓΧΕΙ ΑΝ ΤΟ Ι ΕΙΝΑΙ ΙΔΙΟ ΜΕ ΤΗ ΡΙΖΑ ΤΟΥ NUM
                    {
                    sum+=num/i;
                    }
                }
    ...
    num=sum;//ΑΝΤΙΜΕΤΑΘΕΣΗ ΤΟΥ NUM ΜΕ ΤΟ SUM ΩΣΤΕ ΝΑ ΕΛΕΝΧΤΕΙ ΤΟ ΕΠΟΜΕΝΟ ΑΘΡΟΙΣΜΑ
    count++;
